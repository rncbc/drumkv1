...
#include "drumkv1widget_presets.h"
...
//----------------------------------------------------------------------------
// drumkv1widget_presets::ItemDelegate -- Custom (tree) list item delegate.

class drumkv1widget_presets::ItemDelegate : public QItemDelegate
{
public:
	// ctor.
...
// ctor.
drumkv1widget_presets::ItemDelegate::ItemDelegate (
...
// QItemDelegate interface...
QSize drumkv1widget_presets::ItemDelegate::sizeHint (
...
QWidget *drumkv1widget_presets::ItemDelegate::createEditor ( QWidget *pParent,
	const QStyleOptionViewItem& /*option*/, const QModelIndex& index ) const
{
	...
	case 0: // Text.
	{
		...
		drumkv1widget_presets *pWidget
			= qobject_cast<drumkv1widget_presets *>(parent());
		if (pWidget)
...
#ifdef CONFIG_DEBUG
	qDebug("drumkv1widget_presets::ItemDelegate::createEditor(%p, %d, %d) = %p",
...
void drumkv1widget_presets::ItemDelegate::setEditorData (
	QWidget *pEditor, const QModelIndex& index ) const
{
#ifdef CONFIG_DEBUG
	qDebug("drumkv1widget_presets::ItemDelegate::setEditorData(%p, %d, %d)",
...
void drumkv1widget_presets::ItemDelegate::setModelData ( QWidget *pEditor,
	QAbstractItemModel *pModel,	const QModelIndex& index ) const
{
#ifdef CONFIG_DEBUG
	qDebug("drumkv1widget_presets::ItemDelegate::setModelData(%p, %d, %d)",
		pEditor, index.row(), index.column());
#endif
...
//----------------------------------------------------------------------------
// drumkv1widget_presets -- UI wrapper form.

// ctor.
drumkv1widget_presets::drumkv1widget_presets ( QWidget *pParent )
	: QTreeWidget(pParent)
{
...
// dtor.
drumkv1widget_presets::~drumkv1widget_presets (void)
{
...
// utilities.
void drumkv1widget_presets::loadPresets ( drumkv1_presets *pPresets )
{
	QTreeWidget::clear();
	...
	while (preset_iter.hasNext()) {
		...
		drumkv1_presets::Preset *pPreset = pPresets->find_preset(sPreset);
		if (pPreset == nullptr)
			continue;
		QTreeWidgetItem *pPresetItem = new QTreeWidgetItem(this, PresetItem);
		pPresetItem->setFlags(
			Qt::ItemIsEnabled | Qt::ItemIsEditable | Qt::ItemIsSelectable);
		pPresetItem->setIcon(0, QIcon(":/images/drumkv1_preset.png"));
	...
	while (bank_iter.hasNext()) {
		...
		drumkv1_presets::Bank *pBank = pPresets->find_bank(sBank);
		if (pBank == nullptr)
			continue;
...
void drumkv1widget_presets::savePresets ( drumkv1_presets *pPresets )
{
	...
	for (int iItem = 0 ; iItem < iItemCount; ++iItem) {
		...
		//	const int iBankData = pBankItem->data(0, Qt::UserRole).toInt();
			drumkv1_presets::Bank *pBank = pPresets->add_bank(sBank);
...
void drumkv1widget_presets::loadPresetsCurrent ( drumkv1_presets *pPresets )
{
	QString sBank;
	drumkv1_presets::Bank *pBank
...
void drumkv1widget_presets::savePresetsCurrent ( drumkv1_presets *pPresets )
{
...
QString drumkv1widget_presets::currentPreset (void) const
{
	QString sPreset;
...
// slots.
void drumkv1widget_presets::addBankItem (void)
{
...
void drumkv1widget_presets::addPresetItem (void)
{
	QTreeWidget::setFocus();
...
// factory methods.
QTreeWidgetItem *drumkv1widget_presets::newBankItem (void)
{
	int iBank = 0;
	const int iItemCount = QTreeWidget::topLevelItemCount();
	for (int iItem = 0; iItem < iItemCount; ++iItem) {
...
QTreeWidgetItem *drumkv1widget_presets::newPresetItem (void)
{
...
// file prompts.
//
QStringList drumkv1widget_presets::openPresetFiles (void)
{
	QStringList files;
...
QString drumkv1widget_presets::savePresetFile ( const QString& sPreset )
{
...
// config stuff.
//
void drumkv1widget_presets::setPresetDir ( const QString& sPresetDir )
{
	m_sPresetDir = sPresetDir;
}

const QString& drumkv1widget_presets::presetDir (void) const
{
	return m_sPresetDir;
}


void drumkv1widget_presets::setPresetExt ( const QString& sPresetExt )
{
	m_sPresetExt = sPresetExt;
}

const QString& drumkv1widget_presets::presetExt (void) const
{
	return m_sPresetExt;
}


void drumkv1widget_presets::setDontUseNativeDialogs ( bool bDontUseNativeDialogs )
{
	m_bDontUseNativeDialogs = bDontUseNativeDialogs;
}

bool drumkv1widget_presets::dontUseNativeDialogs (void) const
{
...
QValidator *drumkv1widget_presets::presetsValidator (void) const
{
...
// dirty flag.
void drumkv1widget_presets::setDirtyPresets ( bool bDirtyPresets )
{
	if (bDirtyPresets)
		++m_iDirtyPresets;
	else
...
bool drumkv1widget_presets::isDirtyPresets (void) const
{
...
// private slots.
//
void drumkv1widget_presets::itemChangedSlot ( QTreeWidgetItem *pItem, int )
{
...
void drumkv1widget_presets::itemExpandedSlot ( QTreeWidgetItem *pItem )
{
	if (isBankItem(pItem))
		pItem->setIcon(0, QIcon(":/images/presetBankOpen.png"));
}

void drumkv1widget_presets::itemCollapsedSlot ( QTreeWidgetItem *pItem )
{
	if (isBankItem(pItem))
...
void drumkv1widget_presets::setPresetItem ( const QString& sPreset )
{
...
QTreeWidgetItem *drumkv1widget_presets::presetItem ( const QString& sPreset ) const
{
	return nameItem(sPreset, PresetItem);
}

bool drumkv1widget_presets::isPresetItem ( QTreeWidgetItem *pItem ) const
{
	return (pItem->type() == PresetItem);
}


void drumkv1widget_presets::setBankItem ( const QString& sBank, int iPreset )
{
...
QTreeWidgetItem *drumkv1widget_presets::bankItem ( const QString& sBank ) const
{
	return nameItem(sBank, BankItem);
}

bool drumkv1widget_presets::isBankItem ( QTreeWidgetItem *pItem ) const
{
	return (pItem->type() == BankItem);
}


QTreeWidgetItem *drumkv1widget_presets::nameItem (
...
QString drumkv1widget_presets::nameRenum (
	const QString& sName, int iType ) const
{
...
QString drumkv1widget_presets::bankRenum ( const QString& sBank ) const
{
	return nameRenum(sBank, BankItem);
}


QString drumkv1widget_presets::presetRenum ( const QString& sPreset ) const
{
...
// Drag-n-drop stuff.
//
void drumkv1widget_presets::mousePressEvent ( QMouseEvent *pMouseEvent )
{
...
void drumkv1widget_presets::mouseMoveEvent ( QMouseEvent *pMouseEvent )
{
...
void drumkv1widget_presets::mouseReleaseEvent ( QMouseEvent *pMouseEvent )
{
...
void drumkv1widget_presets::dragEnterEvent ( QDragEnterEvent *pDragEnterEvent )
{
...
void drumkv1widget_presets::dragMoveEvent ( QDragMoveEvent *pDragMoveEvent )
{
...
void drumkv1widget_presets::dragLeaveEvent ( QDragLeaveEvent */*pDragLeaveEvent*/ )
{
	if (m_pRubberBand)
...
void drumkv1widget_presets::dropEvent ( QDropEvent *pDropEvent )
{
	const QPoint& pos
	#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
...
bool drumkv1widget_presets::canDropEvent ( QDropEvent *pDropEvent )
{
...
QTreeWidgetItem *drumkv1widget_presets::dragDropItem ( const QPoint& pos )
{
...
void drumkv1widget_presets::dropItem (
...
void drumkv1widget_presets::moveRubberBand (
	QTreeWidgetItem *pDropItem, bool bOutdent )
{
	// We must be dragging something...
	QTreeWidgetItem *pDragItem = m_pDragItem;
...
void drumkv1widget_presets::ensureVisibleItem ( QTreeWidgetItem *pItem )
{
...
void drumkv1widget_presets::autoExpandTimeoutSlot (void)
{
	m_pAutoExpandTimer->stop();
...
// ctor.
drumkv1widget_presets::ComboBox::ComboBox ( QWidget *pParent )
	: QComboBox(pParent),
	m_pPresetsView(new drumkv1widget_presets(pParent))
{
...
drumkv1widget_presets *drumkv1widget_presets::ComboBox::presetsView (void) const
{
	return m_pPresetsView;
}


void drumkv1widget_presets::ComboBox::setCurrentPreset ( const QString& sPreset )
{
...
QString drumkv1widget_presets::ComboBox::currentPreset (void) const
{
	return QComboBox::currentText();
}


void drumkv1widget_presets::ComboBox::resetCurrentPreset (void)
{
...
// end of drumkv1widget_presets.cpp
