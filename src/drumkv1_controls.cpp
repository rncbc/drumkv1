	...
	bool process ( const drumkv1_controls::Event& event )
	{
		...
		if (event.key.param == RPN_MSB) {
			...
			if (item.is_param_msb()
				|| (item.is_any() && item.type() != drumkv1_controls::RPN))
				enqueue(item);
		...
		if (event.key.param == NRPN_MSB) {
			xrpn_item& item = get_item(channel);
			if (item.is_param_msb()
				|| (item.is_any() && item.type() != drumkv1_controls::NRPN))
				enqueue(item);
			if (item.type() == drumkv1_controls::None) {
		...
		if (event.key.param == DATA_MSB) {
			...
			if (item.is_value_msb()
				|| (item.type() != drumkv1_controls::RPN
		...
		if (event.key.param > CC14_MSB_MIN &&
			...
			if (item.is_param_msb() || item.is_value_msb()
				|| (item.is_any() && item.type() != drumkv1_controls::CC14)
...
void drumkv1_controls::process_event ( const Event& event )
{
	...
	m_sched_out.schedule_event(index, drumkv1_param::paramValue(index, fValue));
}
