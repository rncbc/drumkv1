...
public:
	...
	drumkv1_controls::Type type() const
		{ return drumkv1_controls::Type(status() & 0x700); }
...
public:
	...
	void resize ( unsigned int size )
	{
		...
		if (new_size > m_size) {
			...
			drumkv1_controls::Event *new_events
				= new drumkv1_controls::Event [new_size];
			drumkv1_controls::Event *old_events = m_events;
			if (old_events) {
				if (m_write > m_read) {
					::memcpy(
						(void *) (new_events + m_read),
						(void *) (old_events + m_read),
						(m_write - m_read) * sizeof(drumkv1_controls::Event));
				}
				else
				if (m_write < m_read) {
					::memcpy(
						(void *) (new_events + m_read),
						(void *) (old_events + m_read),
						(old_size - m_read) * sizeof(drumkv1_controls::Event));
					if (m_write > 0) {
						::memcpy(
							(void *) (new_events + old_size),
							(void *) (old_events),
							m_write * sizeof(drumkv1_controls::Event));
					}
					m_write += old_size;
				}
			}
	...
	bool push ( const drumkv1_controls::Event& event )
	{
		if (count() >= m_mask)
			resize(m_size + 4);
	...
	bool pop ( drumkv1_controls::Event& event )
	{
...
private:
	...
	drumkv1_controls::Event *m_events;
};


//---------------------------------------------------------------------
// drumkv1_controls:Impl - decl.
//

class drumkv1_controls::Impl
{
public:
	...
	bool dequeue ( drumkv1_controls::Event& event )
	...
	bool process ( const drumkv1_controls::Event& event )
	{
		const unsigned short channel = event.key.channel();

		if (event.key.param == RPN_MSB) {
			xrpn_item& item = get_item(channel);
			if (item.is_any() && item.type() != drumkv1_controls::RPN)
				enqueue(item);
			if (item.is_status() // RPN nullptr [MSB]
				&& item.type() == drumkv1_controls::RPN && item.is_param_lsb()
				&& item.param_lsb() == 0x7f && event.value == 0x7f) {
				item.clear();
				--m_count;
				return true;
			}
			if (item.type() == drumkv1_controls::NRPN) {
				item.clear();
				item.set_status(drumkv1_controls::RPN | channel);
			}
			else
			if (!item.is_status()) {
				item.set_status(drumkv1_controls::RPN | channel);
		...
		if (event.key.param == RPN_LSB) {
			...
			if (item.is_status() // RPN nullptr [LSB]
				&& item.type() == drumkv1_controls::RPN && item.is_param_msb()
		...
		if (event.key.param == NRPN_MSB) {
			xrpn_item& item = get_item(channel);
			if (item.is_any() && item.type() != drumkv1_controls::NRPN)
				enqueue(item);
			if (item.type() == drumkv1_controls::RPN) {
				item.clear();
				item.set_status(drumkv1_controls::NRPN | channel);
			}
			else
			if (!item.is_status()) {
				item.set_status(drumkv1_controls::NRPN | channel);
		...
		if (event.key.param == DATA_MSB) {
			xrpn_item& item = get_item(channel);
			if (item.type() != drumkv1_controls::RPN &&
				item.type() != drumkv1_controls::NRPN) {
				enqueue(item);
				return false;
			}
			if (!item.is_status())
		...
		if (event.key.param > CC14_MSB_MIN && event.key.param < CC14_MSB_MAX) {
			xrpn_item& item = get_item(channel);
			if (item.is_any() && item.type() != drumkv1_controls::CC14) {
				enqueue(item);
				item.clear();
				--m_count;
			}
			...
			if ((item.is_param_msb() && item.is_value_msb())
				|| (item.type() == drumkv1_controls::CC14 && item.is_param_lsb()
					&& item.param_lsb() != event.key.param + CC14_LSB_MIN))
				enqueue(item);
			if (!item.is_status()) {
				item.set_status(drumkv1_controls::CC14 | channel);
				++m_count;
			}
		...
		if (event.key.param > CC14_LSB_MIN && event.key.param < CC14_LSB_MAX) {
			...
			if ((item.is_param_lsb() && item.is_value_lsb())
				|| (item.type() == drumkv1_controls::CC14 && item.is_param_msb()
...
protected:
	...
	void enqueue ( xrpn_item& item )
	{
		...
		if (item.type() == drumkv1_controls::CC14) {
			if (item.is_14bit()) {
			...
			//	--m_count;
				...
				const unsigned short status
					= drumkv1_controls::CC | item.channel();
				if (item.is_param_msb() && item.is_value_msb())
...
drumkv1_controls::~drumkv1_controls (void)
{
	delete m_pImpl;
}
...
void drumkv1_controls::process_dequeue (void)
{
	if (!enabled())
		return;
...
void drumkv1_controls::process_event ( const Event& event )
{
	...
	bool bSync = (data.flags & Hook) || !drumkv1_param::paramFloat(index);
	if (!bSync)
		bSync = data.sync;
	if (!bSync) {
	...
	if (bSync) {
		m_sched_out.schedule_event(index,
			drumkv1_param::paramValue(index, fScale));
	}
}
...
void drumkv1_controls::reset (void)
{
	if (!enabled())
		return;
...
// text utilities.
drumkv1_controls::Type drumkv1_controls::typeFromText ( const QString& sText )
{
	if (sText == "CC")
		return CC;
	else
...
QString drumkv1_controls::textFromType ( Type ctype )
{
	QString sText;
