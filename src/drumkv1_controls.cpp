...
public:
	...
	drumkv1_controls::Type type() const
		{ return drumkv1_controls::Type(m_status & 0xf00); }
	...
	bool process ( const drumkv1_controls::Event& event )
	{
		...
		if (event.key.param == RPN_MSB) {
			...
			if (item.is_any() && item.type() != drumkv1_controls::RPN)
				enqueue(item);
			if (item.type() == drumkv1_controls::None) {
		...
		if (event.key.param == NRPN_MSB) {
			...
			if (item.is_any() && item.type() != drumkv1_controls::NRPN)
				enqueue(item);
			if (item.type() == drumkv1_controls::RPN && item.is_param_lsb()
		...
		if (event.key.param == NRPN_LSB) {
			...
			if (item.is_any() && item.type() != drumkv1_controls::NRPN)
				enqueue(item);
			if (item.type() == drumkv1_controls::RPN && item.is_param_msb()
		...
		if (event.key.param > CC14_MSB_MIN &&
			...
			if (item.is_param_msb() || item.is_value_msb()
				|| (item.type() == drumkv1_controls::CC14
