...
public:
	...
	void start()
		{ pthread_create(&m_pthread, NULL, drumkv1_alsa_thread::run, this); }
...
int drumkv1_jack::process ( jack_nframes_t nframes )
{
	...
	const float *bpm_sync = drumkv1::paramPort(drumkv1::DEL1_BPMSYNC);
	if (bpm_sync && *bpm_sync > 0.0f) {
		float *bpm_port = drumkv1::paramPort(drumkv1::DEL1_BPM);
		if (bpm_port) {
			jack_position_t pos;
