...
QString drumkv1_config::presetsGroup (void) const
{
	return "/Presets";
}
...
QString drumkv1_config::presetsBanksGroup (void) const
{
	return "/Banks";
}

QString drumkv1_config::presetsBankListKey (void) const
{
	return "/BankList";
}

QString drumkv1_config::presetsCurrentGroup (void) const
{
	return "/Current";
}
...
void drumkv1_config::load (void)
{
	...
	while (bank_iter.hasNext()) {
		...
		drumkv1_presets::Bank *pBank = presets.add_bank(sBank);
	...
	while (preset_iter.hasNext()) {
		const QString& sPreset = preset_iter.next();
		drumkv1_presets::Preset *pPreset = presets.find_preset(sPreset);
		if (pPreset == nullptr)
...
void drumkv1_config::save (void)
{
	...
	while (bank_iter.hasNext()) {
		const QString& sBank = bank_iter.next();
		drumkv1_presets::Bank *pBank = presets.find_bank(sBank);
		if (pBank == nullptr)
			continue;
	...
	QSettings::beginGroup(presetsCurrentGroup());
	drumkv1_presets::Bank *pBank = presets.current_bank();
	drumkv1_presets::Preset *pPreset = presets.current_preset();
