...
void drumkv1_config::importPresets (
	const QString& sFilename, drumkv1_presets *pPresets )
{
...
void drumkv1_config::loadPresets ( QSettings *pSettings,
...
void drumkv1_config::loadPresets (void)
{
	presets.clear_banks();
...
void drumkv1_config::exportPresets (
	const QString& sFilename, drumkv1_presets *pPresets )
{
	QSettings settings(sFilename, QSettings::IniFormat);
...
void drumkv1_config::savePresets (
	QSettings *pSettings, drumkv1_presets *pPresets, const MapPath& mapPath )
{
	...
	const drumkv1_presets::Presets& presets_map
		= pPresets->presets();
	drumkv1_presets::Presets::ConstIterator presets_iter
		= presets_map.constBegin();
	const drumkv1_presets::Presets::ConstIterator& presets_end
...
void drumkv1_config::savePresets (void)
{
...
//
int drumkv1_config::loadPresetsConf (
	QSettings *pSettings, drumkv1_presets *pPresets )
{
	int nconfs = 0;
...
int drumkv1_config::loadPresetsConfDir (
	drumkv1_presets *pPresets, const QDir& dir, QStringList& confs )
{
	int nconfs = 0;
	...
	while (iter.hasNext()) {
		...
		if (!confs.contains(sFilename)) {
			drumkv1_config::importPresets(sFilename, pPresets);
