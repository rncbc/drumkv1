...
public:
	...
	drumkv1_control::Type type() const
		{ return drumkv1_control::Type(m_status & 0xf0); }
...
public:
	...
	void resize ( unsigned int size )
	{
		...
		if (new_size > m_size) {
			...
			drumkv1_control::Event *new_events
				= new drumkv1_control::Event [new_size];
			drumkv1_control::Event *old_events = m_events;
			if (old_events) {
				if (m_write > m_read) {
					::memcpy(new_events + m_read, old_events + m_read,
						(m_write - m_read) * sizeof(drumkv1_control::Event));
				}
				else
				if (m_write < m_read) {
					::memcpy(new_events + m_read, old_events + m_read,
						(old_size - m_read) * sizeof(drumkv1_control::Event));
					if (m_write > 0) {
						::memcpy(new_events + old_size, old_events,
							m_write * sizeof(drumkv1_control::Event));
					}
					m_write += old_size;
				}
			}
	...
	bool push ( const drumkv1_control::Event& event )
	{
		if (count() >= m_mask)
			resize(m_size + 4);
	...
	bool pop ( drumkv1_control::Event& event )
	{
...
private:
	...
	drumkv1_control::Event *m_events;
};


//---------------------------------------------------------------------
// drumkv1_control:Impl - decl.
//
class drumkv1_control::Impl
{
public:
	...
	bool dequeue ( drumkv1_control::Event& event )
	...
	bool process ( const drumkv1_control::Event& event )
	{
		...
		if (event.param == RPN_MSB) {
			...
			if (item.is_param_msb()
				|| (item.is_any() && item.type() != drumkv1_control::RPN))
				enqueue(item);
			if (item.type() == drumkv1_control::None) {
				item.set_status(drumkv1_control::RPN | channel);
				++m_count;
			}
		...
		if (event.param == NRPN_MSB) {
			xrpn_item& item = get_item(channel);
			if (item.is_param_msb()
				|| (item.is_any() && item.type() != drumkv1_control::NRPN))
				enqueue(item);
			if (item.type() == drumkv1_control::None) {
				item.set_status(drumkv1_control::NRPN | channel);
				++m_count;
			}
			item.set_param_msb(event.value);
			return true;
		...
		if (event.param == DATA_MSB) {
			...
			if (item.is_value_msb()
				|| (item.type() != drumkv1_control::RPN
				 && item.type() != drumkv1_control::NRPN)) {
				enqueue(item);
				return false;
			}
		...
		if (event.param > CC14_MSB_MIN && event.param < CC14_MSB_MAX) {
			...
			if (item.is_param_msb() || item.is_value_msb()
				|| (item.is_any() && item.type() != drumkv1_control::CC14)
				|| (item.type() == drumkv1_control::CC14
					&& item.param_lsb() != event.param + CC14_LSB_MIN))
				enqueue(item);
			if (item.type() == drumkv1_control::None) {
				item.set_status(drumkv1_control::CC14 | channel);
				++m_count;
			}
...
protected:
	...
	void enqueue ( xrpn_item& item )
	{
		...
		if (item.type() == drumkv1_control::CC14) {
			if (item.is_14bit()) {
				m_queue.push(item.status(), item.param_msb(), item.value());
			} else  {
				const unsigned char status = drumkv1_control::CC | item.channel();
				if (item.is_value_msb())
		...
		if (item.is_ready()) {
			...
			const unsigned char status = drumkv1_control::CC | item.channel();
			if (item.type() == drumkv1_control::RPN) {
				if (item.is_param_msb())
			...
			if (item.type() == drumkv1_control::NRPN) {
				if (item.is_param_msb())
...
//---------------------------------------------------------------------
// drumkv1_control - impl.
//

drumkv1_control::drumkv1_control ( drumkv1 *pDrumk )
	: m_pImpl(new drumkv1_control::Impl()), m_pDrumk(pDrumk)
{
}


drumkv1_control::~drumkv1_control (void)
{
	delete m_pImpl;
}
...
void drumkv1_control::process_enqueue (
...
void drumkv1_control::process_dequeue (void)
{
	Event event;
...
void drumkv1_control::process_event ( const Event& event )
{
	const Key key(event);
	...
	m_pDrumk->setParamValue(drumkv1::ParamIndex(index), fValue);
}


void drumkv1_control::flush (void)
{
	m_pImpl->flush();
}


// end of drumkv1_control.cpp
