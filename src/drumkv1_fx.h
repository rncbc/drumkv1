...
inline float drumkv1_fx_tanhf ( const float x )
{
	const float x2 = x * x;
...
//-------------------------------------------------------------------------
// drumkv1_fx_filter - RBJ filter implementation.
...
public:
	...
	drumkv1_fx_comp(uint32_t iSampleRate = 44100)
	...
	void reset()
	{
		...
		// HOUSE eq.
		m_lo.reset(drumkv1_fx_filter::Peak,      100.0f, m_srate, 1.0f, 6.0f);
		m_mi.reset(drumkv1_fx_filter::LoShelf,  1000.0f, m_srate, 1.0f, 3.0f);
		m_hi.reset(drumkv1_fx_filter::HiShelf, 10000.0f, m_srate, 1.0f, 4.0f);
	}
	...
	float saturate(float x, float t) const
	{
		...
		if(x > 0.0f)
			return +(t + (1.0f - t) * drumkv1_fx_tanhf((+x - t) / (1.0f - t)));
		else
			return -(t + (1.0f - t) * drumkv1_fx_tanhf((-x - t) / (1.0f - t)));
	}
...
public:
	...
	drumkv1_fx_chorus(uint32_t iSampleRate = 44100)
...
public:
	...
	drumkv1_fx_delay(uint32_t iSampleRate = 44100)
...
public:
	...
	drumkv1_fx_phaser(uint32_t iSampleRate = 44100)
		: m_srate(float(iSampleRate)) { reset(); }
