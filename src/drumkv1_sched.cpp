...
protected:
	...
	{
		return static_cast<drumkv1_sched_thread *> (arg)->run();
	}
...
// ctor.
drumkv1_sched_thread::drumkv1_sched_thread ( uint32_t nsize )
{
	m_nsize = (4 << 1);
	while (m_nsize < nsize)
		m_nsize <<= 1;
...
// real thread start executive.
void drumkv1_sched_thread::start (void)
{
	pthread_create(&m_thread, NULL, drumkv1_sched_thread::run, this);
}
