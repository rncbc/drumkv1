...
void drumkv1widget_param::setSingleStep ( float fSingleStep )
{
	m_pDial->setSingleStep(scaleFromValue(fSingleStep));
}


float drumkv1widget_param::singleStep (void) const
{
...
// Dial change slot.
void drumkv1widget_param::dialValueChanged ( int iDialValue )
{
	...
	: drumkv1widget_param(pParent)
{
	...
	const QFontMetrics fm(drumkv1widget_param::font());
...
{
	drumkv1widget_param::setValue(valueFromScale(float(spinValue)));
}
...
{
	drumkv1widget_param::setValue(float(iComboValue));
}
